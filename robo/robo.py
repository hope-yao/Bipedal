...
def step_cycle(state,_time):
    ...
    while cnt<300:
        ...
        and (chain_num==3) and (np.min(np.abs(state[-num_time:,2]-state[-num_time:,4]))<np.radians(1)):
            ...
            # state = np.insert(tmp,[0],state,axis=0)
            chain_num = 2
        ...
        if (chain_num==2) and cosd<(3): # in degree
            ...
            print(np.degrees([state[-1,0],state[-1,2],state[-1,4]]))
            # switching nonstance leg and stance leg at this moment
            # continue
            break
        # three linked chain
        if (chain_num==3):
        ...
        ab = np.dot(v1, v2)
        print(ab)
        cosd = [np.degrees(math.acos( 0.999* ab[i] / np.linalg.norm((x_nsf_tmp[i],y_nsf_tmp[i])))) for i in range(num_time)]
    ...
    return x_h,y_h,x_nsk,y_nsk,x_nsf,y_nsf,state[-1,:]
...
def animate(i):
    ...
    # x_bipedal = [0, x_h[-10], x_nsk[-10], x_nsf[-10]]
    # y_bipedal = [0, y_h[-10], y_nsk[-10], y_nsf[-10]]
...
x_h, y_h, x_nsk, y_nsk, x_nsf, y_nsf,state = step_cycle(state, dt)
# for i in range(2):
#     x_h_, y_h_, x_nsk_, y_nsk_, x_nsf_, y_nsf_, state = step_cycle(state, dt)
#     x_h.append(x_h_)
